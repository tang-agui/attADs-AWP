...
def random_brightness(image, bright_coeff=-1):
    verify_image(image)
    #   revised by TGJ
    if(bright_coeff != -1):
        if(bright_coeff < -1.0 or bright_coeff > 1.0):
            raise Exception(err_brightness_coeff)
    ...
    if(is_list(image)):
        ...
        for img in image_list:
            if(bright_coeff == -1):
                ...
                random_brightness_coefficient = 1 + bright_coeff
...
def add_shadow(image, no_of_shadows=1, rectangular_roi=(-1, -1, -1, -1), shadow_dimension=3):
    verify_image(image)
    ...
    elif x1 == -1 or y1 == -1 or x2 == -1 or y2 == -1:
        raise Exception(err_invalid_rectangular_roi)
    #   revise by TGJ
    if x1 >=  x2:
        x1 = x1-1
...
def add_fog(image, fog_coeff=-1):
    ...
    if(is_list(image)):
        ...
        if fog_coeff == -1:
            ...
            fog_coeff_t = fog_coeff
        hw = int((imshape[1]//3) * fog_coeff_t)  # [H, W, C]
        #   TGJ revised in 2023-03-28
        # if hw < 10:
        #     hw = 10
        # print("fog_coeff_t : ", fog_coeff_t, '/n', "hw : ", hw)
        #   H//3 = 202
        haze_list = generate_random_blur_coordinates(imshape, hw)
        for haze_points in haze_list:
            image = add_blur(
...
def add_sun_process(image, no_of_flare_circles, flare_center, src_radius, x, y, src_color):
    ...
    for i in range(no_of_flare_circles):
        ...
        cv2.circle(overlay, (int(x[r]), int(y[r])), rad*rad*rad, (random.randint(max(src_color[0]-50, 0), src_color[0]),
                                                                  random.randint(max(src_color[1]-50, 0), src_color[1]), random.randint(max(src_color[2]-50, 0), src_color[2])), -1)
        cv2.addWeighted(overlay, alpha, output, 1 - alpha, 0, output)
...
def add_sun_flare(image, flare_center=-1, angle=-1, no_of_flare_circles=1, src_radius=100, src_color=(255, 255, 255)):
    ...
    if(is_list(image)):
        ...
        if flare_center == -1:
            ...
            flare_center_t = (random.randint(
                0, imshape[1]), random.randint(0, imshape[0]))
        else:
            flare_center_t = flare_center
        x, y = add_sun_flare_line(flare_center_t, angle_t, imshape)
        output = add_sun_process(
